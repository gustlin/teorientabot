...
contextList = "(Lang=Pt-BR) Seu papel é criar uma lista de exercícios sobre o assunto dado, não use comandos Latex"
...
)  #Set intents as defined in the discord API website
...
@client.event
async def on_ready():
    print('Connected as {0.user}'.format(
        client))  #Confirm bot is running correctly
...
lvalue = "O comando !lista é utilizado para gerar uma lista de exercícios sobre um tema específico. Ao usar este comando, o bot criará uma lista de exercícios com questões variadas e de diferentes níveis de dificuldade (Você está livre para especificar)."
...
@client.command()
async def help(ctx):
    em = discord.Embed(title="Lista de Comandos",
    ...
    em.add_field(name='!lista', value=lvalue, inline=False)
...
def get_chatgpt_response(prompt, context, mod):
    completion = chatgpt.chat.completions.create(model=mod,
                                                 messages=[{
                                                     "role": "system",
                                                     "content": context
                                                 }, {
...
@client.command()
async def lista(ctx, *, prompt):
    prompt = str(prompt)
    answer = get_chatgpt_response(prompt, contextList, gpt3)
    await ctx.send(answer)
...
async def imagem(ctx):
    ...
    for attachment in ctx.message.attachments:
        if attachment.filename.lower().endswith(
            ...
            await ctx.send(
                'Por favor mande um arquivo do formato adequado (png, jpg, jpeg, gif, bmp).'
            )
